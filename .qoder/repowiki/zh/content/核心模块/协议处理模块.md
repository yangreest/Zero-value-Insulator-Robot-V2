...
## 更新摘要
**变更内容**
- 新增舵机到位反馈功能，支持精确的运动控制状态监控
- 实现 CServoArrivalFeedback 类用于解析和验证舵机到位状态
- 添加 0x1A 命令帧的完整处理逻辑和数据解析
- 集成回调注册机制，支持上层业务对舵机到位事件的响应
- 增强运动控制精度，提供到位偏差检测和角度转换功能
...
## 简介
本模块实现 WHSD 控制板通信协议，负责命令封装、数据解析、响应分发、心跳机制、OTA 升级以及传感器/校准数据的回调。**最新增强了舵机到位反馈功能**，通过 CServoArrivalFeedback 类提供精确的运动控制状态监控，支持正常到位和超时异常的完整处理流程。通过统一的接口与设备通信层（TCP）协作，向上层 UI 和业务逻辑提供稳定可靠的控制与数据采集能力。
...
Proto --> Servo["舵机反馈<br/>CServoArrivalFeedback"]
```
...
- **新增数据结构**
  ...
  - **CServoArrivalFeedback：舵机到位反馈数据（结果、目标位置、实际位置）**
...
## 架构总览
协议层以"接收数据 -> 解析 -> 分发"为主线，配合"定时心跳 -> 设备状态上报"的双向通道。**新增舵机到位反馈通道**，通过 0x1A 命令帧实现精确的运动控制状态监控。上层通过回调注册将协议事件映射到 UI 或业务逻辑。
...
UI->>P : 注册回调(答案/日志/心跳/零值/传感器/校准/舵机到位/OTA)
UI->>P : BeginWork()
P->>C : 周期性发送心跳(0x00)
D-->>C : 心跳应答(0x00, 64字节)
C-->>P : ReceiveNewData(...)
...
P->>C : Write(封装后的帧)
D-->>C : 响应帧(0x0F/0x11/0x17/0x1A/OTA状态)
C-->>P : ReceiveNewData(...)
P->>UI : 触发 ZeroData/SensorData/CalibAnswer/**ServoArrival**/OTAStatus
```
...
- **舵机到位反馈（0x1A）**：**新增功能**，解析结果、目标位置、实际位置，回调 ServoArrival。
...
- [WHSDControlBoradProtocol.cpp:784-802](file://Insulator_Zero_Value_Detection_Robot/Protocol/WHSDControlBoradProtocol.cpp#L784-802)
...
**章节来源**
- [WHSDControlBoradProtocol.cpp:631-667](file://Insulator_Zero_Value_Detection_Robot/Protocol/WHSDControlBoradProtocol.cpp#L631-667)
- [IDeviceCom.h:4-15](file://Insulator_Zero_Value_Detection_Robot/DeviceCom/IDeviceCom.h#L4-L15)
- [TcpClient.h:8-46](file://Insulator_Zero_Value_Detection_Robot/DeviceCom/TcpClient.h#L8-L46)
- [Insulator_Zero_Value_Detection_Robot.cpp:277-290](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L277-L290)
...
+m_cResult uint8_t
+m_wTargetPos uint16_t
+m_wActualPos uint16_t
+IsArrived() bool
+StepsToAngle(wSteps) double
}
...
CWHSDControlBoardProtocol --> CServoArrivalFeedback : "使用"
...
Note over D,P : 舵机到位反馈流程
D-->>P : 0x1A 舵机到位反馈
P->>UI : ServoArrivalCallback(feedback)
```
...
Dispatch --> Case0x1A{"命令字 = 0x1A?"}
Case0x1A --> |是| ParseServo["解析舵机到位反馈"]
Case0x1A --> |否| OtherCases["其他命令处理"]
ParseServo --> Done(["结束"])
OtherCases --> Done
```
...
- 可靠性：
  ...
  - **舵机到位精度保障**：通过 IsArrived() 方法提供 ±20步（约±1.8°）的到位判定阈值，确保运动控制精度。
...
- **舵机到位异常**：
  - 检查 0x1A 命令帧是否正确解析，确认数据域长度至少5字节。
  - 验证目标位置与实际位置的偏差是否在合理范围内（≤20步）。
  - 确认回调函数已正确注册，检查 IsArrived() 方法的判定逻辑。
  - 观察日志输出，区分正常到位（0x01）和超时异常（0x00）情况。
- **电源状态异常**：
  - 检查首次心跳是否正常到达。
...
## 结论
该协议处理模块以清晰的帧格式、严格的校验与健壮的错误恢复机制，实现了与 WHSD 控制板的可靠通信。通过回调机制将协议事件解耦到上层业务，结合心跳与 OTA 能力，满足工业现场对稳定性与可维护性的要求。**最新的舵机到位反馈功能显著增强了运动控制的精确性**，通过 CServoArrivalFeedback 类提供完整的到位状态监控，支持正常到位和超时异常的差异化处理，并结合角度转换和偏差检测功能，为精密运动控制提供了可靠的技术支撑。**同时保持了原有的安全初始化机制**，确保在首次心跳消息到达前系统处于安全的默认状态，有效防止了未初始化值导致的潜在风险。建议在实际部署中完善日志记录与监控指标，以便快速定位问题。

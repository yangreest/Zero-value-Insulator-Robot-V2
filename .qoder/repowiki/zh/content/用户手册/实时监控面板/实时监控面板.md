# 实时监控面板

<cite>
**本文引用的文件**
- [main.cpp](file://Insulator_Zero_Value_Detection_Robot/main.cpp)
- [Insulator_Zero_Value_Detection_Robot.h](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.h)
- [Insulator_Zero_Value_Detection_Robot.cpp](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp)
- [Insulator_Zero_Value_Detection_Robot.ui](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.ui)
- [contentwidget.h](file://Insulator_Zero_Value_Detection_Robot/UI/contentwidget.h)
- [contentwidget.cpp](file://Insulator_Zero_Value_Detection_Robot/UI/contentwidget.cpp)
- [modeldatawidget.h](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.h)
- [modeldatawidget.cpp](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.cpp)
- [modeldatamodel.h](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatamodel.h)
- [modeldatamodel.cpp](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatamodel.cpp)
</cite>

## 更新摘要
**所做更改**
- 新增综合告警弹窗系统章节，详细介绍三种严重级别的实时弹窗通知机制
- 更新异常情况报警提示章节，反映新的智能告警显示逻辑
- 增强故障诊断指南，包含告警弹窗系统的排障方法
- 更新监控面板使用技巧，增加告警弹窗处理相关操作

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与刷新机制](#性能与刷新机制)
8. [故障诊断指南](#故障诊断指南)
9. [结论](#结论)
10. [附录：监控面板使用技巧与配置](#附录：监控面板使用技巧与配置)

## 简介
本文件面向"绝缘子零值检测机器人V2"的实时监控面板，聚焦于界面中设备状态、电池电量、通信质量、检测进度等实时信息的展示与交互。文档从系统架构、数据流、刷新频率、告警提示、历史数据查看与分析等方面，提供可操作的使用说明与排障建议，帮助用户高效掌握监控面板的各项功能。**最新更新**：系统新增了综合告警弹窗系统，提供三种严重级别的实时通知，支持颜色编码视觉指示和自动隐藏功能。

## 项目结构
本项目采用Qt框架构建桌面应用，主窗口负责UI布局与事件绑定，协议层负责与下位机通信，摄像头模块负责视频流显示，图表与表格用于实时数据可视化。关键入口与UI初始化如下：
- 程序入口创建主窗口并进入事件循环
- 主窗口初始化参数、绑定定时器与控件动作、建立通信与协议回调、启动摄像头取流
- UI通过QSplitter划分视频区与标签页区域，支持拖拽调整比例

```mermaid
graph TB
A["main.cpp<br/>创建应用与主窗口"] --> B["Insulator_Zero_Value_Detection_Robot.cpp<br/>InitParam/InitUI/BindAction"]
B --> C["IDeviceCom + 协议层<br/>心跳/传感器/测量结果回调"]
B --> D["摄像头模块<br/>视频流显示到label_9/label_22"]
B --> E["ModelDataWidget<br/>表格+曲线实时更新"]
B --> F["Insulator_Zero_Value_Detection_Robot.ui<br/>界面布局与控件"]
B --> G["告警弹窗系统<br/>三级严重级别通知"]
```

**图示来源**
- [main.cpp:11-23](file://Insulator_Zero_Value_Detection_Robot/main.cpp#L11-L23)
- [Insulator_Zero_Value_Detection_Robot.cpp:88-95](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L88-L95)
- [Insulator_Zero_Value_Detection_Robot.cpp:104-229](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L104-L229)
- [Insulator_Zero_Value_Detection_Robot.ui:19-350](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.ui#L19-L350)

**章节来源**
- [main.cpp:11-23](file://Insulator_Zero_Value_Detection_Robot/main.cpp#L11-L23)
- [Insulator_Zero_Value_Detection_Robot.cpp:88-229](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L88-L229)
- [Insulator_Zero_Value_Detection_Robot.ui:19-350](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.ui#L19-L350)

## 核心组件
- 主窗口类：负责UI初始化、定时器、通信注册、摄像头连接、测量流程控制、告警与工单/报告管理。
- 模型与视图：ModelDataModel/ModelDataWidget实现表格与曲线的联动更新；ContentWidget作为图表容器基类。
- 通信与协议：通过IDeviceCom与WHSDControlBoardProtocol接收心跳、传感器数据、测量结果，并转发至UI线程更新。
- 摄像头：根据配置选择新旧摄像头接入方式，将视频帧渲染到指定Label。
- **综合告警弹窗系统**：集中式告警管理，支持时间戳记录、严重级别分类、详细信息追踪和实时弹窗通知。

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.h:26-385](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.h#L26-L385)
- [Insulator_Zero_Value_Detection_Robot.cpp:231-315](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L231-L315)
- [modeldatawidget.h:20-49](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.h#L20-L49)
- [modeldatawidget.cpp:24-120](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.cpp#L24-L120)
- [contentwidget.h:12-32](file://Insulator_Zero_Value_Detection_Robot/UI/contentwidget.h#L12-L32)
- [contentwidget.cpp:17-63](file://Insulator_Zero_Value_Detection_Robot/UI/contentwidget.cpp#L17-L63)

## 架构总览
实时监控面板的数据流由"设备→协议→UI"构成，UI侧通过定时器轮询与回调驱动刷新。

```mermaid
sequenceDiagram
participant Dev as "下位机"
participant Com as "IDeviceCom"
participant Proto as "WHSDControlBoardProtocol"
participant UI as "主窗口(Insulator_Zero_Value_Detection_Robot)"
participant Chart as "ModelDataWidget"
participant Alarm as "告警弹窗系统"
Note over UI,Dev : 定时轮询传感器数据
loop 每1秒约
UI->>Com : 发送传感器查询命令
Com-->>Proto : 透传数据
Proto-->>UI : 回调传感器数据(状态/电量/结果)
UI->>UI : 更新状态/电量/结果标签
end
Note over UI,Dev : 测量结果回报
Dev-->>Proto : 上报测量值(0x0F)
Proto-->>UI : 回调测量值
UI->>Chart : appendValue(按列追加)
Chart-->>UI : 曲线与表格同步更新
Note over UI,Alarm : 异常时触发告警弹窗
UI->>Alarm : AddAlarm() + ShowAlarmPopup()
Alarm-->>UI : 顶部弹窗通知(4秒自动消失)
```

**图示来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:413-424](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L413-L424)
- [Insulator_Zero_Value_Detection_Robot.cpp:649-673](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L649-L673)
- [Insulator_Zero_Value_Detection_Robot.cpp:675-753](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L675-L753)
- [Insulator_Zero_Value_Detection_Robot.cpp:3460-3529](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L3460-L3529)
- [modeldatawidget.cpp:139-166](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.cpp#L139-L166)

## 详细组件分析

### 设备状态与通信质量指示
- 连接状态：通过通信回调更新"已连接/未连接"，并在界面标签显示。
- 心跳计数：累计心跳次数，便于判断通信是否持续。
- 电机与安全状态：解析心跳中的行走电机与安全电机状态，转换为"停止/运行/故障"等文本。
- 电源与电池：显示主电源开关状态与电池百分比；若数值异常则显示"未知"。

```mermaid
flowchart TD
Start(["定时器触发"]) --> Query["发送传感器查询命令"]
Query --> Receive{"收到传感器数据?"}
Receive -- 否 --> Wait["等待下一次轮询"]
Receive -- 是 --> Update["更新状态/电量/结果标签"]
Update --> End(["结束"])
```

**图示来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:413-559](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L413-L559)

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:413-559](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L413-L559)
- [Insulator_Zero_Value_Detection_Robot.cpp:635-640](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L635-L640)

### 电池电量显示
- 电池电量来自心跳或传感器回调，超过阈值时显示"未知"，否则显示具体数值。
- 支持在设置中保存绝缘阈值，影响低值告警判定。

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:486-503](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L486-L503)
- [Insulator_Zero_Value_Detection_Robot.cpp:649-673](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L649-L673)

### 检测进度跟踪与结果展示
- 测量结果到达后，叠加显示当前检测结果，并写入工单测量数据JSON。
- 表格与曲线同步更新：每获得一个测量值，填充对应列的空单元格，并绘制曲线点；纵轴范围动态扩展。
- 双联模式：内侧/外侧分别记录，表头加入侧别维度，便于区分。

```mermaid
sequenceDiagram
participant Proto as "协议层"
participant UI as "主窗口"
participant Chart as "ModelDataWidget"
participant Table as "表格"
participant Curve as "曲线"
Proto-->>UI : 测量结果回调
UI->>UI : 计算侧别/相别索引
UI->>Chart : appendValue(列名, 值)
Chart->>Table : 填充下一个空单元格
Chart->>Curve : 添加数据点并扩展Y轴
UI->>UI : 更新工单起止时间与落盘
```

**图示来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:675-753](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L675-L753)
- [modeldatawidget.cpp:139-166](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.cpp#L139-L166)

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:675-753](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L675-L753)
- [modeldatawidget.cpp:139-166](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.cpp#L139-L166)

### 综合告警弹窗系统
**新增功能** 系统实现了统一的告警管理机制，通过AddAlarm()方法和ShowAlarmPopup()方法提供集中化的告警处理和实时弹窗通知。

#### 三种严重级别分类
系统定义了三种告警严重级别，每种级别都有对应的视觉标识：

- **严重级别（Critical）**：红色背景 (#D32F2F)，用于零值/低值检测缺陷
- **警告级别（Warning）**：橙色背景 (#EF6C00)，用于测量异常影响流程的情况  
- **提示级别（Info）**：蓝色背景 (#1565C0)，用于一般性提示信息

#### 智能告警映射机制
系统根据告警类型自动映射到相应严重级别：
- "零值/低值" → 严重级别（检测到的缺陷）
- "测量异常" → 警告级别（影响测量流程）
- 其他类型 → 提示级别（一般信息）

#### 实时弹窗通知特性
- **位置定位**：弹窗固定在界面顶部中央，宽度为主窗口的2/3
- **自动隐藏**：4秒后自动消失，避免干扰用户操作
- **智能覆盖**：新告警到达时覆盖旧内容并重新计时
- **非模态设计**：不拦截鼠标事件，不影响正常操作
- **视觉优化**：圆角边框、白色文字、粗体字体、内边距优化

#### 告警表结构与历史记录
告警表位于"告警信息"标签页，包含以下列：
- **时间**：精确的时间戳（yyyy-MM-dd HH:mm:ss格式）
- **类型**：告警类别（如"零值/低值"、"测量异常"）
- **位置**：告警发生的设备位置或相别
- **详情**：详细的告警描述信息
- **状态**：处理状态（默认"未处理"）

#### 告警触发场景
系统在以下场景中自动生成告警：
- **测量异常**：当控制板未连接或总电源未开启时，无法开始检测
- **零值/低值**：测量值低于绝缘阈值时生成告警
- **重测失败**：重测过程中遇到异常情况
- **探针到位超时**：探针到位等待超时导致测量中止

```mermaid
flowchart TD
A["系统事件发生"] --> B{"事件类型判断"}
B -- "测量异常" --> C["调用AddAlarm('测量异常', 位置, 原因)"]
B -- "零值/低值" --> D["调用AddAlarm('零值/低值', 位置, 详情)"]
B -- "其他异常" --> E["调用AddAlarm('其他异常', 位置, 描述)"]
C --> F["插入告警表首行"]
D --> F
E --> F
F --> G["记录设备日志"]
G --> H["显示顶部弹窗通知"]
H --> I["4秒后自动隐藏"]
```

**图示来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:3460-3529](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L3460-L3529)
- [Insulator_Zero_Value_Detection_Robot.cpp:766](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L766)
- [Insulator_Zero_Value_Detection_Robot.cpp:2121](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L2121)
- [Insulator_Zero_Value_Detection_Robot.cpp:2239](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L2239)
- [Insulator_Zero_Value_Detection_Robot.cpp:2504](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L2504)

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.h:237-253](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.h#L237-L253)
- [Insulator_Zero_Value_Detection_Robot.cpp:3460-3529](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L3460-L3529)
- [Insulator_Zero_Value_Detection_Robot.ui:4560-4693](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.ui#L4560-L4693)

### 历史数据的查看与分析
- 工单与报告列表支持过滤与重置，便于回顾历史任务。
- 每次测量值写入工单JSON，并自动更新起止时间；支持删除与重测以修正数据。
- 曲线与表格提供直观的历史趋势对比，支持按列（侧别/相别）观察。
- **告警历史**：告警表保留完整的告警历史记录，支持按时间顺序查看和分析。

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:710-753](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L710-L753)
- [modeldatawidget.cpp:71-120](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.cpp#L71-L120)

## 依赖关系分析
- 主窗口依赖通信接口与协议层进行数据收发，依赖摄像头模块进行视频显示，依赖图表与表格组件进行数据可视化。
- 定时器驱动轮询与输入处理，确保界面响应及时。
- **告警系统依赖**：告警系统与设备日志系统紧密集成，所有告警同时记录到日志文件；弹窗系统依赖QTimer实现自动隐藏功能。

```mermaid
classDiagram
class Insulator_Zero_Value_Detection_Robot {
+On_timer_timeout()
+CallBack_SensorValue()
+CallBack_ZeroValue()
+CameraConnect()
+AppendMearData()
+AddAlarm(strType, strLocation, strDetail)
+ShowAlarmPopup(eLevel, strText)
+AlarmLevelFromType(strType)
}
class IDeviceCom {
+Write(data)
+RegisterReadDataCallBack(cb)
+RegisterConnectStatusCallBack(cb)
}
class WHSDControlBoardProtocol {
+ReceiveNewData()
+RegisterAnswerFunction(cb)
+RegisterDeviceHeartBeat(cb)
+RegisterSensorDataCallBack(cb)
+RegisterZeroDataCallBack(cb)
}
class ModelDataWidget {
+appendValue(header, value)
+removeLastValue(header)
}
class CWriteLog {
+Write(message)
}
class QTimer {
+setInterval(ms)
+start()
+stop()
}
Insulator_Zero_Value_Detection_Robot --> IDeviceCom : "发送/接收数据"
Insulator_Zero_Value_Detection_Robot --> WHSDControlBoardProtocol : "协议回调"
Insulator_Zero_Value_Detection_Robot --> ModelDataWidget : "更新表格/曲线"
Insulator_Zero_Value_Detection_Robot --> CWriteLog : "记录告警日志"
Insulator_Zero_Value_Detection_Robot --> QTimer : "自动隐藏弹窗"
```

**图示来源**
- [Insulator_Zero_Value_Detection_Robot.h:26-385](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.h#L26-L385)
- [Insulator_Zero_Value_Detection_Robot.cpp:231-315](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L231-L315)
- [modeldatawidget.h:20-49](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.h#L20-L49)

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.h:26-385](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.h#L26-L385)
- [Insulator_Zero_Value_Detection_Robot.cpp:231-315](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L231-L315)
- [modeldatawidget.h:20-49](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.h#L20-L49)

## 性能与刷新机制
- 定时器周期：主定时器100ms，输入定时器10ms；传感器查询约每秒一次，避免频繁通信造成负载。
- 数据更新策略：仅在收到有效数据时更新UI；曲线纵轴动态扩展，减少重绘开销。
- 录像与截图：录制期间缓存帧，停止后统一编码；截图文件名含内测/外侧标记与时间戳。
- **告警性能优化**：告警表采用insertRow(0)方式将新告警插入首行，确保最新告警始终可见；弹窗系统使用懒创建模式，只在首次使用时创建QLabel和QTimer对象；表头自动拉伸以适应不同屏幕尺寸。

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:317-329](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L317-L329)
- [Insulator_Zero_Value_Detection_Robot.cpp:413-424](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L413-L424)
- [Insulator_Zero_Value_Detection_Robot.cpp:3492-3529](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L3492-L3529)
- [modeldatawidget.cpp:139-166](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.cpp#L139-L166)

## 故障诊断指南
- 连接异常：检查"已连接/未连接"标签与心跳计数；确认IP与端口配置正确。
- 无传感器数据：确认定时器是否工作、传感器查询命令是否发出；查看日志输出。
- 测量结果不更新：检查回调是否切到UI线程；确认表格列名与侧别/相别匹配。
- 告警过多：核对绝缘阈值设置；确认是否为真实低值情况。
- **告警弹窗系统故障排查**：
  - 检查弹窗是否正常显示：确认m_pAlarmPopupLabel是否正确初始化
  - 验证严重级别映射：检查AlarmLevelFromType()方法是否正确识别告警类型
  - 测试自动隐藏功能：确认QTimer::timeout信号是否正确连接到隐藏槽函数
  - 检查弹窗样式：验证CSS样式表是否正确设置背景色和字体属性
  - 验证弹窗位置：确认move()方法计算的坐标是否正确居中显示
  - 检查内存管理：确保弹窗对象正确创建和销毁，避免内存泄漏

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:413-559](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L413-L559)
- [Insulator_Zero_Value_Detection_Robot.cpp:635-640](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L635-L640)
- [Insulator_Zero_Value_Detection_Robot.cpp:755-767](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L755-L767)
- [Insulator_Zero_Value_Detection_Robot.cpp:3492-3529](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L3492-L3529)

## 结论
实时监控面板通过定时器轮询与协议回调实现了设备状态、电池电量、通信质量与检测进度的实时展示。结合表格与曲线，用户可直观跟踪检测过程并进行历史数据分析。合理的刷新频率与数据更新策略保障了界面的流畅性与准确性。**最新的综合告警弹窗系统**提供了三种严重级别的实时通知机制，支持颜色编码视觉指示和智能自动隐藏功能，显著提升了系统的可维护性和问题定位效率。通过告警机制与故障诊断指南，用户可及时发现并处理问题，提升运维效率。

## 附录：监控面板使用技巧与配置
- 自定义显示内容与布局
  - 通过QSplitter调整视频区与标签页比例，适应不同屏幕尺寸。
  - 表格列宽按表头文字宽度自适应，可通过拖拽微调。
- 刷新频率与时效性
  - 传感器查询约每秒一次，测量结果到达后立即更新；曲线纵轴动态扩展，保证最新数据可见。
- 历史数据回顾
  - 使用工单与报告列表的过滤功能快速定位历史任务；曲线与表格支持对比分析。
- **告警弹窗系统使用技巧**
  - 注意观察界面顶部的彩色弹窗通知，不同颜色代表不同严重级别
  - 红色弹窗表示严重问题（零值/低值），需要立即关注和处理
  - 橙色弹窗表示警告信息（测量异常），需要检查相关配置
  - 蓝色弹窗表示提示信息，了解系统运行状态
  - 弹窗会在4秒后自动消失，不会干扰正常操作
  - 定期检查"告警信息"标签页，及时处理"未处理"状态的告警
  - 利用告警的时间戳信息快速定位问题发生时间
  - 结合设备日志文件进行深入的问题分析
  - 根据告警类型和位置信息快速定位故障设备
- 常见问题排查
  - 连接状态为"未连接"：检查网络与IP配置；查看心跳计数是否增长。
  - 无测量结果：确认测量流程是否触发；检查回调是否切到UI线程。
  - 告警误报：核对绝缘阈值设置；确认是否为真实低值情况。
  - 弹窗不显示：检查告警弹窗初始化是否正常；确认ShowAlarmPopup方法调用是否正确
  - 弹窗样式异常：检查CSS样式表是否正确加载；验证颜色代码格式是否正确

**章节来源**
- [Insulator_Zero_Value_Detection_Robot.cpp:104-229](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L104-L229)
- [modeldatawidget.cpp:71-120](file://Insulator_Zero_Value_Detection_Robot/UI/modeldatawidget.cpp#L71-L120)
- [Insulator_Zero_Value_Detection_Robot.cpp:413-559](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L413-L559)
- [Insulator_Zero_Value_Detection_Robot.cpp:3460-3529](file://Insulator_Zero_Value_Detection_Robot/UI/Insulator_Zero_Value_Detection_Robot.cpp#L3460-L3529)
...
## 更新摘要
**变更内容**
- 更新了版本控制配置，新增对视频文件和截图的忽略规则
- 增强了仓库卫生管理，排除大型二进制媒体文件
- 完善了系统设置配置的完整说明
...
**更新** 版本控制配置已增强，现在自动忽略视频文件（*.avi）和截图文件（截图/截图.png），提高仓库管理效率。
...
- [.gitignore:77-78](file://.gitignore#L77-L78)
...
- 性能监控
  ...
  - 可通过日志分析网络延迟与错误。
- **版本控制优化**
  - .gitignore已增强，自动忽略*.avi视频文件和截图/截图.png文件
  - 提高仓库管理效率，避免大型二进制媒体文件占用版本控制空间
...
- 相机与视频
  ...
  - 视频文件（*.avi）已被.gitignore自动忽略，不会占用版本控制空间。
- 配置变更
  - 修改相机IP后需重启软件；修改设备IP后立即生效并重连。
  - 探针角度与绝缘阈值影响测量精度，谨慎调整并验证。
- **版本控制优化**
  - 利用.gitignore规则自动排除大型媒体文件，提高开发效率。
  - 截图文件（截图/截图.png）也被忽略，避免不必要的版本控制开销。
...
- 配置保存失败
  ...
  - 查看日志中保存失败的提示。
- **版本控制问题**
  - 如果视频文件或截图被意外提交到版本库，检查.gitignore配置是否正确。
  - 使用`git rm --cached`命令移除已跟踪的文件。
...
**更新** 版本控制配置已优化，通过.gitignore规则自动排除大型媒体文件，提高开发效率和仓库管理质量。

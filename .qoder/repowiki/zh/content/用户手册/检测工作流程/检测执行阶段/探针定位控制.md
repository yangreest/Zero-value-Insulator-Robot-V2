...
## 更新摘要
**变更内容**
- 探针到位超时时间从4秒延长至8秒，提升机械运动检测可靠性
- 探针到位检测逻辑重构为基于定时器状态判断而非原子标志位，解决UI线程与硬件通信的竞态条件问题
- 传感器轮询命令被注释禁用，减少网络通信冲突
- 增强舵机到位反馈处理机制，通过定时器活动状态判断流程上下文
...
**更新** 新增了基于定时器状态判断的智能定位控制，通过改进的到位检测机制和延长的超时时间，显著提升了系统的可靠性和稳定性。
...
- 测量流程状态机（UI线程）
  ...
  - On_ProbeWaitTick：周期检查定时器状态或兜底超时，触发测量
...
**更新** 改进了到位检测机制，使用定时器活动状态判断替代原子标志位，解决了UI线程与硬件通信的竞态条件问题。
...
Proto-->>Dev : 写入帧
Dev-->>UI : 心跳/状态可选到位信号
UI->>UI : On_ProbeWaitTick 轮询定时器状态或超时
...
**更新** 重构了到位检测逻辑，使用定时器活动状态判断替代原子标志位，通过 `m_pProbeWaitTimer->isActive()` 判断是否在测量等待流程中，解决了UI线程与硬件通信的竞态条件问题。

```mermaid
stateDiagram-v2
[*] --> 空闲
空闲 --> 移动到等待到位 : "StartProbeMoveAndWait"
移动到等待到位 --> 精确定位 : "On_ProbeWaitTick 定时器超时"
精确定位 --> 复原 : "结果到达/超时"
复原 --> [*]
```
...
### 到位检测机制
- 轮询策略
  - 每 PROBE_ARRIVE_POLL_MS (100ms) 检查一次定时器状态或累计耗时
  - 若超过 PROBE_ARRIVE_TIMEOUT_MS (8000ms) 则触发测量
- 安全兜底
  - 若通信断开或总电源已确认关闭，立即中止测量并复原探针
- 结果超时
  - 测量指令发出后启动 MEASURE_RESULT_TIMEOUT_MS (8000ms) 定时器，未收到 0x0F 则自动结束

**更新** 延长了探针到位超时时间从4秒至8秒，并通过定时器活动状态判断替代原子标志位，显著提升了机械运动检测的可靠性和系统稳定性。

```mermaid
flowchart TD
S["开始等待到位"] --> P["周期轮询: 检查定时器状态/耗时"]
P --> J{"是否超时?"}
...
**更新** 增强了舵机到位反馈处理机制，通过定时器活动状态判断确保仅在测量等待流程中处理反馈，避免了非预期情况下的误操作。
...
**更新** 增强了安全保护措施，通过定时器活动状态判断确保仅在正确的流程上下文中处理舵机到位反馈，提升了系统的整体安全性。
...
**更新** 通过延长到位超时时间和改进检测逻辑，显著提升了定位性能和系统稳定性，减少了因通信延迟导致的误判。
...
**更新** 新增了基于定时器状态判断的故障排查指导，包括定时器活动状态检查、超时时间调优等方法。
...
## 结论
探针定位控制通过清晰的状态机、可靠的定时器轮询机制与完善的异常处理，实现了稳定高效的定位与测量流程。结合配置管理与单点定标，可在不同工况下保持良好精度与鲁棒性。新增的基于定时器状态判断的检测机制进一步提升了定位精度和系统可靠性，延长的超时时间有效应对了机械运动的延迟问题。建议在实际部署中根据现场环境调优速度与超时参数，并定期维护标定数据。
...
- 参数建议
  ...
  - PROBE_ARRIVE_TIMEOUT_MS：根据机械响应时间设定，当前设置为8秒以提升可靠性
  - MEASURE_RESULT_TIMEOUT_MS：略大于 0x0F 典型回报时间（约3.2秒），当前设置为8秒

**更新** 新增了基于定时器状态判断的调试方法，包括定时器活动状态监控、超时时间优化等调试技巧。
